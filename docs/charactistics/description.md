---
title: Hermes Agent 特性總覽與使用指引
description: 以產品級文件方式整理 Hermes Agent 的核心能力、資料落點、安全基線、日常應用與效能排查。
---

## 摘要

Hermes Agent 的核心價值不在於「聊天更像人」，而在於把 **可持久化的知識（facts）與可重複執行的流程（procedures）** 系統化：當它成功完成一項任務，能將做法沉澱為可重用技能，並透過排程與整合把成熟流程自動化，讓整體成本隨使用次數遞減。

Hermes 所謂「跟使用者一起成長」並非對 base model 做 fine-tuning，而是透過以下四層持久化能力實現：

- **長期記憶（精簡、可控）**：`MEMORY.md` / `USER.md`
- **完整會話歷史（可搜尋、可回溯）**：`~/.hermes/state.db`（FTS5）
- **技能文件（可重用 SOP / runbook）**：`~/.hermes/skills/`
- **自動化（把流程變成例行）**：cron / webhook / messaging gateway

---

## 目錄

- [1. 定位：Hermes Agent 是什麼](#1-定位hermes-agent-是什麼)
- [2. 核心能力總覽](#2-核心能力總覽)
- [3. 「越用越懂你」的運作機制](#3-越用越懂你的運作機制)
- [4. 資料落點與匯出/備份](#4-資料落點與匯出備份)
- [5. Everyday tasks：可落地的日常場景](#5-everyday-tasks可落地的日常場景)
- [6. Profile 設計建議（權限與資料隔離）](#6-profile-設計建議權限與資料隔離)
- [7. 安全基線（Defense-in-Depth）](#7-安全基線defense-in-depth)
- [8. 效能與延遲排查（常見「很慢」的根因）](#8-效能與延遲排查常見很慢的根因)
- [9. 操作要點：關閉、續聊與常駐](#9-操作要點關閉續聊與常駐)
- [參考資料](#參考資料)

---

## 1. 定位：Hermes Agent 是什麼

Hermes Agent 更接近一個 **local-first 的 agent runtime**。其設計通常由以下能力組成（實際可用內容依版本與設定而定）：

- **工具執行層**：terminal / file / browser / integrations（依 toolsets 啟用）
- **持久化層**：memory、skills、sessions（state database）
- **自動化層**：cron、webhook、messaging gateway
- **委派機制**：subagent / delegation（依設定啟用）

因此，Hermes 最能發揮價值的使用方式不是「長時間對話」，而是把高頻、可標準化的工作流沉澱成技能與排程，並持續壓低重工率。

---

## 2. 核心能力總覽

### 2.1 閉環自學與技能沉澱（Skills / Self-Improving）

- **技能化（procedural memory）**：當 Hermes 成功完成一個高成本任務（例如：複雜除錯、跨系統資料擷取、CI 失敗分析），可將可重用的方法寫成 `SKILL.md`，後續遇到類似問題可直接套用，降低反覆推理成本。[^skills]
- **可演化的工作流**：若你啟用自我進化相關模組，系統可基於過往軌跡與成功案例，優化技能與提示詞，使 agent 在「不重新訓練模型」的前提下迭代工作流。（此項目是否適用，取決於你的專案需求與啟用的元件。）

### 2.2 跨會話持久記憶（Persistent Memory）

Hermes 以兩個可控、體積小的檔案承載「熱記憶」：

- **`USER.md`**：使用者偏好（語言、輸出格式、決策習慣、工作節奏）
- **`MEMORY.md`**：專案/環境事實、工具 quirks、完成工作、修正紀錄

設計原則是 **短而精**：避免把原始資料 dump 進記憶，改記「可持久重用」的要點（例如：repo 慣例、部署流程、常見坑與修復順序）。[^memory]

### 2.3 多平台閘道與自動化（Gateway / Cron）

- **跨平台入口一致**：可透過 CLI 與常見 messaging 平台下指令，讓 agent 在不同設備/環境間維持一致操作方式。[^sessions]
- **排程化**：把例行任務（夜間備份、每週審計、每日摘要）固化成 cron；輸出可回傳原對話或推送到指定通道。[^cron]

### 2.4 深度防禦（Defense-in-Depth）

因為 agent 可能具有檔案/終端機權限，實務上建議把安全視為系統性能力，而不是單一開關。常見做法包含：

- **危險指令需顯式授權**（避免誤刪、破壞性操作）
- **隔離執行**：偏好 Docker / sandbox 後端降低對 host 的破壞面
- **上下文掃描**：降低 prompt injection 與秘密外洩風險（例如：誘導讀 `.env` 或上傳憑證）

（實際可用層級與行為，依 Hermes 版本與 toolsets/config 而定。）[^config]

### 2.5 跨模型與資料隱私（Model Agnostic / Data Privacy）

- **供應商不綁定**：可切換不同 LLM provider，亦可接本地/自架 endpoint（OpenAI-compatible API）。
- **Local-first ≠ 零出網**：若使用雲端 provider，prompt（包含注入進 prompt 的記憶）仍會送往 provider；高敏感情境建議改用本地/內部 gateway，並縮小工具與資料注入面。[^faq]

---

## 3. 「越用越懂你」的運作機制

Hermes 的「成長」可拆成四個層次，每層都應有明確責任邊界：

| 層次 | 主要承載 | 存什麼 | 主要用途 |
| --- | --- | --- | --- |
| 使用者偏好（User Profile） | `USER.md` | 語氣、格式、輸出模板、決策偏好 | 讓回覆更貼合個人習慣 |
| 專案/環境記憶（Agent Memory） | `MEMORY.md` | repo 慣例、環境事實、修正事項 | 降低重複交代背景成本 |
| 會話歷史（Session History） | `state.db` | 完整訊息與工具呼叫 | 需要時回查「之前做過什麼」 |
| 程序技能（Procedures / Skills） | `skills/` | 可重用 SOP / runbook | 類似 playbook：直接套用成功做法 |

### 3.1 建議記憶的內容（高價值）

- **偏好**：語言、摘要長度、章節格式、輸出檔案型態（例如偏好 deliverables）
- **環境事實**：OS / shell、常用工具、專案技術棧、限制條件
- **專案慣例**：分支命名、commit style、CI pipeline、發布流程
- **修正紀錄**：踩過的坑、已證實不可行的做法、替代方案

### 3.2 不建議記憶的內容（高風險/低效益）

- **秘密/憑證**：API keys、bot token、OAuth 憑證、SSH key、`.env` 原文
- **原始資料 dump**：大量 log、整段程式碼、內網拓樸細節（可改成摘要）
- **易變資訊**：暫時路徑、一次性任務中間產物（除非會重複使用）
...
（實際目錄與內容請以你的版本為準；此處為文件化整理。）[^contrib]
...
（備份與還原行為以 CLI 文件為準。）[^cli]

#### B. 匯出會話為 JSONL（建議：審計 / redaction / 歸檔）
...
（會話與匯出格式請以 sessions 文件為準。）[^sessions]

#### C. 只匯出「人可讀」的記憶與技能（建議：知識交接）
...
**安全提醒**：避免把 `~/.hermes/.env`、`~/.hermes/auth.json`、`state.db`、`sessions/` 直接上傳到公開 repo。

---

## 5. Everyday tasks：可落地的日常場景

以下示例的重點是「可自動化」與「可沉澱」：每個場景都應能產出技能或固定排程，而非一次性對話。

### 5.1 每日 briefing / research radar
...
以繁體中文摘要，每則 2 句內，附來源連結，推送到 Telegram。
```

### 5.2 GitHub / Coding / DevSecOps

可落地的例行任務：

```text
- 夜間 issue triage
- PR 自動 review（含風險與測試建議）
- CI failure analysis（輸出修復步驟與回歸風險）
- dependency / security audit
- docs drift detection
```

（可參考官方 automation templates。）[^templates]

### 5.3 DevOps / On-call 協作

```text
- uptime check / smoke test
- alert triage（比對近期部署/設定變更）
- 例行審計（磁碟、retention、備份可用性）
```

### 5.4 Gmail / Calendar / Docs（建議與 coding 權限分離）
...
## 6. Profile 設計建議（權限與資料隔離）

若你的使用範圍同時涵蓋「個人資料」與「工作 repo/終端機」，建議用多 profile 進行隔離，避免：

- 個人資訊進入 coding session 的上下文
- coding 工具權限被個人助理場景誤用
...
- **`personal`**：Google Workspace / 提醒 / briefing（限制終端機與檔案權限）
- **`coder`**：repo automation / CI debug（允許 terminal/file/GitHub/MCP）
- **`security-lab`**：監控與告警（允許 webhook/SSH/terminal，但嚴格輸出去識別）
- **`study`**：學習與筆記（允許 web/notes/PDF，避免接觸工作 secrets）

（profiles 行為以官方文件為準。）[^profiles]

---

## 7. 安全基線（Defense-in-Depth）

### 7.1 最小權限原則（建議落地做法）

- **工具最小化**：只啟用當下任務需要的 toolsets
- **隔離執行**：偏好 Docker/sandbox backend（尤其是 terminal/file）
- **資料去識別**：對 logs、告警、ESG/客戶資料做匿名化/遮罩後再輸出到外部通道

### 7.2 備份與分享的紅線

不要直接分享（或上傳到公開 GitHub）：
...
~/.hermes/state.db
~/.hermes/sessions/
任何含 API key、bot token、客戶名稱、內網 IP、機密路徑的 memory/skill/session
```
...
以下是常見根因排序，適用於多數「本地跑 agent」或「工具全開」的情境：

1. **模型不適合 agent loop / tool calling**（例如偏純聊天模型）
2. **context 過大**（prefill / KV cache 成本暴增；本地端尤其明顯）
3. **工具/技能載入過多**（tool schema + skills 導致 prompt 膨脹）
4. **一開始就全開 browser automation / cron / messaging / code execution**（首 token 延遲上升）
5. **硬體不匹配**（低 VRAM/CPU 以大模型 + 長上下文做即時互動，體驗通常不佳）

### 8.1 建議的「從小開始」啟動方式

先用小 toolset 起跑（需要查網頁再加 web）：
...
若使用 Ollama 類本地服務，通常建議先把 context 控制在 16K–32K 的合理區間，再逐步調整模型大小與工具集。
...
### 9.2 「有長期記憶」與「續接同一段對話」是兩件事

- **重新開 `hermes chat`**：會帶入 `USER.md` / `MEMORY.md`（長期記憶），但通常是新 session
- **續接同一段對話**：使用 `--continue`（或 `-c`）/ `--resume <SESSION_ID>` 以載回完整 history

### 9.3 需要常駐（關掉 Terminal 也要繼續跑）

若要長期接 webhook / cron / messaging gateway，建議使用：

- **tmux/screen**：快速可用
- **service/daemon（如 launchd/systemd）**：較正式且可控

---

## 參考資料

[^github]: `https://github.com/nousresearch/hermes-agent`（Hermes Agent GitHub）
[^memory]: `https://hermes-agent.nousresearch.com/docs/user-guide/features/memory`（Persistent Memory）
[^sessions]: `https://hermes-agent.nousresearch.com/docs/user-guide/sessions`（Sessions）
[^skills]: `https://hermes-agent.nousresearch.com/docs/user-guide/features/skills`（Skills System）
[^cron]: `https://hermes-agent.nousresearch.com/docs/user-guide/features/cron`（Scheduled Tasks / Cron）
[^templates]: `https://hermes-agent.nousresearch.com/docs/guides/automation-templates`（Automation Templates）
[^profiles]: `https://hermes-agent.nousresearch.com/docs/user-guide/profiles`（Profiles）
[^config]: `https://hermes-agent.nousresearch.com/docs/user-guide/configuration`（Configuration）
...
- **技能化（procedural memory）**：當 Hermes 成功完成一個高成本任務（例如：複雜除錯、跨系統資料擷取、CI 失敗分析），可將可重用的方法寫成 `SKILL.md`，後續遇到類似問題可直接套用，降低反覆推理成本。[^skills]
- **可演化的工作流**：若你啟用自我進化相關模組，系統可基於過往軌跡與成功案例，optimization技能與提示詞，使 agent 在「不重新訓練模型」的前提下迭代工作流。（此項目是否適用，取決於你的專案需求與啟用的元件。）
...
自我進化機制：借助專屬的 hermes-agent-self-evolution 模組，系統會利用 DSPy 與 GEPA（Genetic-Pareto Prompt Evolution）算法，根據過去的執行軌跡，自動optimization其擁有的技能、系統提示詞與代碼。這使 Agent 能在不依賴 GPU 重新訓練模型的狀況下，隨著使用次數增加而自動提升效能與準確率。
...
對.的 GitLab / MinIO / Prometheus / Grafana lab，可以變成：
...
# 4. 建議.的 Hermes profile 設計

基於.的使用模式，建議至少拆 4 個 profiles：
...
官方 configuration 文件提醒：local backend 會以.的 user account 權限直接跑命令；若不想讓 agent 有同等 filesystem access，應停用不需要的 tools 或改用 Docker backend。([Hermes Agent][15])
...
真正要讓它「跟.一起成長」，關鍵不是一直聊天，而是固定讓它沉澱：
...
# 1. .的 constraint 下，最實際的 provider 排序
...
| 5   |             **Claude subscription 直接接 Hermes** |               低 |   快 | 通常不符合.的限制；Hermes 端需要 Max extra usage 或 API key |
...
# 4. 如果.有 GitHub Copilot：這可能是最穩的「已付費訂閱」解法
...
.現在 `131.1K context` 對 local agent 來說太重。先改成 32K，比較合理。
...
# 或 3B. 如果.有 Copilot，選 GitHub Copilot
# 或 3C. 如果.要本地，選 Custom endpoint -> Ollama
...
第三：OpenAI Codex provider，如果.的 ChatGPT/Codex 帳號支援
...
.現在是用前景 CLI process 跑：

python3 ~/.local/bin/hermes chat --model hermes3

Terminal 關掉後，這個 process 通常會一起結束；除非.是用 tmux、screen、nohup、launchd、systemd、cron gateway、messaging gateway 之類方式讓它常駐。
